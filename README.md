...
The aim of this project is to build an automaton that recognizes a language defined by a regular expression, and to test word membership using the automaton:

* In the first part, we'll develop a lexical analyzer to recognize the various tokens needed to run the application. (i.e. regexp.l file)

* In the second part, we'll define our axioms and their production rules in the file regexp.y in order to recognize our regular expression for building our automaton, as well as our words that we'll need to test and finally generate a main file that we'll compile. 
